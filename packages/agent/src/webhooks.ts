...
type WebhookTestSuccess = components["schemas"]["WebhookTestSuccess"];
...
export interface TestWebhookClient {
  POST(
    path: "/agents/me/webhooks/test",
    init: Record<string, never>,
  ): Promise<{ data?: WebhookTestSuccess; error?: unknown }>;
}

export async function testWebhookDelivery(
  client: TestWebhookClient,
): Promise<WebhookTestSuccess> {
  const { data, error } = await client.POST("/agents/me/webhooks/test", {});
  if (error || !data) {
    throw new Error(`testWebhookDelivery: POST failed: ${JSON.stringify(error)}`);
  }
  return data;
}
