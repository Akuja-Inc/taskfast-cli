...
#[tokio::test]
async fn list_mine_omits_status_param_and_passes_server_page_through() {
    // #121: the server owns the active-status classification. With no
    // `--status`, the CLI sends no `status` param and returns the server's
    // page verbatim — no client-side filtering or re-paging. The
    // `query_param_is_missing` matcher fails the request (→ 404) if the CLI
    // ever leaks a `status` param on the default path.
    let server = MockServer::start().await;
    let body = json!({
        "data": [
    ...
    let v = envelope_value(&envelope);
    let statuses: Vec<String> = v["data"]["tasks"]
        .as_array()
        .expect("tasks array")
        .iter()
        .map(|t| t["status"].as_str().unwrap().to_string())
        .collect();
    // Returned verbatim — the CLI neither drops nor reorders the server page.
